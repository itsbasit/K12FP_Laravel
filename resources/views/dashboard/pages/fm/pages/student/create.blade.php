...
<div class="row">
    <div class="col-md-12 grid-margin"> 
        <div class="card">
            <div class="card-body"> 
                <form class="forms-sample" method="POST" enctype="multipart/form-data" action="{{url('fm/student/store')}}">
                <h4 class="mb-5">Create New Page</h4>
                @csrf
                <div class="row">
                    <div class="com-md-8">
                        <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <div class="form-group fundraisers">
                            ...
                            <option value="{{$_data->id}}" {{ (old("student") == $_data->name ? "selected":"") }}>{{$_data->name}}</option>
                            @endforeach
                            </select>
                            @error('student')
                    ...
                    <div class="col-md-6">
                            ...
                            <input type="text" value="{{old('slug')}}" name="slug" placeholder="Enter Page title" class="slug form-control {{$errors->has('slug') ? 'is-invalid':''}}">
                            @error('slug')
                                <label id="name-error" class="error invalid-feedback" for="name">{{$message}}</label>
                            @enderror
                            </div>
                ...
                <div class="col-md-4">
                        <div class="form-group fundraisers">
                            ...
                            <option value="{{$_data->id}}" {{ (old("fundraiser") == $_data->name ? "selected":"") }}>{{$_data->name}}</option>
                            @endforeach
                            </select>
                            @error('fundraiser')
                                    <label id="name-error" class="error invalid-feedback" for="name">{{$message}}</label>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" name="student_goal" value="{{old('student_goal')}}" class="form-control student_goal {{$errors->has('student_goal') ? 'is-invalid':''}}" placeholder="Fundraiser Each Student Goal $ ">
                            @error('student_goal')
                                    <label id="name-error" class="error invalid-feedback" for="name">{{$message}}</label>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" name="team" value="{{old('team')}}" class="form-control team {{$errors->has('team') ? 'is-invalid':''}}" placeholder="Enter name of Team, Academi Club, Booster Club, Other Cause etc">
                            @error('team')
                                    <label id="name-error" class="error invalid-feedback" for="name">{{$message}}</label>
                            @enderror
                        </div>
